...
>`).scale("C2:chromatic").sound("gm_fx_atmosphere").room(2).lpf(500)
  .hpf(300).attack(0.15).release(1).sustain(1).decay(0.75).gain("<0 0.07 0.03>")
...
>`).sound("gm_violin").scale("C4:chromatic")
  .release(0.5).attack(0.1).lpf(1500).sustain(0.85)
  .gain("<0.45 0.35>*2").room(1).decay(0.5)
let backvocal = n(`<
{~} 
{~}
{~ [f#4 e4] [f#4 e4] ~ ~ [f#4 e4] [f#4 e4@2 d4]@2}
>`).sound("gm_violin").scale("C4:chromatic")
  .release(0.5).attack(0.1).lpf(600).sustain(0.85)
  .gain("<0.3 0.15 0.3 0.15>*4").room(1.5).decay(0.7)
$:
stack(
  drums._punchcard(), 
  bkgd._punchcard(), 
