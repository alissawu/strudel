...
// intro
let drums = s("tr808_bd:3, tr808_bd:6")
  .struct(`<
  { 1 [~@7 1] 1 1}
  >`).gain("<0.3 0.2 0.3 0.2>*4").sustain(0.5)
let bkgd = n(`<
  {f#4 g4 f#4 e4}
...
>`).sound("gm_violin").scale("C4:chromatic")
  .release(0.4).attack(0.1).lpf(1200).sustain(0.85)
  .gain("<0.15 0.1 0.15 0.1>*4").room(1.5).decay(0.7)
...
let bkgd1 = n(`<
  {f#4 g4 f#4 e4}
>`).scale("C2:chromatic").sound("gm_pad_warm:6").room(2).lpf(500)
  .hpf(300).attack(0.5).release(1.5).sustain(1).decay(0.75).gain(0.02)
  .lpenv(-3).pan(cosine.slow(2))
let bass1 = n(`<
...
let drums1 = s("tr808_bd:1, [~ tr808_bd:6]*2")
  .struct(`<
  { [1 1] [[~ 1] 1]}
  >`).gain("<0.4 0.2 0.3 0.2>*4").lpf(200).sustain(0.5).fast(4)
let piano1 = n(`<
...
>`).sound("gm_violin").scale("C4:chromatic")
  ...
  .gain(0.4).room(1)
let bassch = n(`<
{b1 b1 [b1 d2] [b1 a1] g1 g1 [g1 f#1] [g1 a1] d2 d2 d2 f#2 a1 [a1 e1] a1 c#2}!2
>`)
  .scale("C2:chromatic")
  .sound("supersaw").sustain(0.7)
  .release(0.1)
...
let drumsch = s("[~ dmx_sd:3]*2, [tr808_bd:6 ~]*2")
  .struct(`<
  { 1 [1@3 1] [1 1] 1}
  >`).gain("<0.3 0.1 0.2 0.1>*4").sustain(0.7).release(0.3).fast(4).postgain(0.2)
